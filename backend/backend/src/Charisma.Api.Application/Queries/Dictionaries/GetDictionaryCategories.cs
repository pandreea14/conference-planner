...
{
    public class GetDictionaryCategories
    {
        ...
        public class QueryHandler(IDictionaryRepository dictionaryRepository) : IRequestHandler<Query, IEnumerable<DictionaryItemDto>>
        {
            ...
            {
                var dictionaryCategories = await dictionaryRepository.GetDictionaryCategories(cancellationToken);

                return dictionaryCategories.Select(item => new DictionaryItemDto
                {
                    Id = item.Id,
                    Code = item.Code,
                    Name = item.Name
