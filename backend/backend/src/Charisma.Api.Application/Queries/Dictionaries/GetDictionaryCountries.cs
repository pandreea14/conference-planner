...
{
    public class GetDictionaryCountries
    {
        ...
        public class QueryHandler(IDictionaryRepository dictionaryRepository) : IRequestHandler<Query, IEnumerable<DictionaryItemDto>>
        {
            ...
            {
                var dictionaryCountries = await dictionaryRepository.GetDictionaryCountries(cancellationToken);

                return dictionaryCountries.Select(item => new DictionaryItemDto
                {
                    Id = item.Id,
                    Code = item.Code,
                    Name = item.Name
