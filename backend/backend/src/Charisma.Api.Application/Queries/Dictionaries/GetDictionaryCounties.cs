...
{
    public class GetDictionaryCounties
    {
        ...
        public class QueryHandler(IDictionaryRepository dictionaryRepository) : IRequestHandler<Query, IEnumerable<DictionaryItemDto>>
        {
            ...
            {
                var dictionaryCounties = await dictionaryRepository.GetDictionaryCounties(cancellationToken);

                return dictionaryCounties.Select(item => new DictionaryItemDto
                {
                    Id = item.Id,
                    Code = item.Code,
                    Name = item.Name
